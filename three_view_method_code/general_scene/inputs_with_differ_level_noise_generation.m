...
clear;
%---------------------------------------------------------------------------------------------
addpath([pwd,'\PointClasses_subfunc'],[pwd,'\PoseClasses_subfunc'],[pwd,'\plotAxesAndPoints_subfunc'],[pwd,'\auxiliary_subfunc']);
data_file_name = '_200poses';
load([pwd,'\ground_truth_data\ground_truth',data_file_name,'.mat']);
XYZcolors = 'rgbcmy';
axeslength = 0.1;                                        % 1.5
...
for sigma_s = 0.001:0.001:0.015                              % meter
    ...
    if is_first == 1
        save([pwd,'\input_data\sensor_measurements',data_file_name,'.mat'],'camera_image_list','sonar_image_list','camera_image_list_pixel','camera_intrinsics');
        is_first = 0;
    end
        save([pwd,'\input_data\sensor_measurements_noise_(mean_',num2str(mean_s),'_sigma_',num2str(sigma_s),')',data_file_name,'.mat'],'camera_image_list_noise','camera_image_list_pixel_noise','sonar_image_list_noise','camera_intrinsics');
    disp('-------------------------Finished-------------------------');
end